...
export const Trust = () => (
  <section id="trust" className="relative py-20 sm:py-28">
    <div className="container-x">
      ...
      <Reveal delay={0.1}>
        <div className="mt-14 grid grid-cols-2 gap-8 rounded-3xl glass p-10 sm:gap-6 lg:grid-cols-4">
          {STATS.map((s) => (
            <Stat key={s.label} {...s} />
          ))}
        </div>
    ...
    {/* Editorial marquee */}
    <div className="relative mt-16 overflow-hidden border-y border-white/5 py-6" aria-hidden="true">
      <div className="flex w-max animate-marquee gap-10" style={{ willChange: "transform" }} data-testid="trust-marquee">
        {[...MARQUEE, ...MARQUEE].map((item, i) => (
          <span key={i} className="flex items-center gap-10 whitespace-nowrap font-display text-xl font-semibold text-white/25 sm:text-2xl">
            {item}
